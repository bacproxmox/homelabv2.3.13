...
echo "🩺 Homelab v2.3.13 health check"
echo
...
check_http Nextcloud http://192.168.50.104:8080
...
rssh 107 'nvidia-smi >/dev/null 2>&1 && echo "nvidia-smi OK" || echo "nvidia-smi missing"; mounted=$(findmnt -rn -o TARGET | grep -E "^/mnt/chia-plots/disk[0-9]+$" | sort -V | wc -l); echo "mounted plot disks=${mounted} / expected=6"; if [ "$mounted" -lt 6 ]; then echo "⚠️ 6 plot disk bekleniyor; eksik disk/passthrough/cable/power kontrol et."; fi; df -h | grep /mnt/chia-plots || true; grep -R "parallel_decompressor_count" ~/.chia/mainnet/config/config.yaml 2>/dev/null || true; ss -ltnp | grep 55400 || true' || true
