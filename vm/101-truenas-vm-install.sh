...
# Canonical Homelab v2.3.13 hardware IDs. Do not ask unless hardware really changed.
...
🧊 Homelab v2.3.13 - TrueNAS VM 101 oluşturma
Storage              : $STORAGE
...
if qm status "$VMID" >/dev/null 2>&1; then
  ...
  qm create "$VMID"     --name "$VMNAME"     --memory "$TRUENAS_VM_RAM"     --cores 4     --cpu host     --machine q35     --bios ovmf     --scsihw virtio-scsi-single     --net0 virtio,bridge=vmbr0     --onboot 1     --balloon 0     --vga vmware
fi
...
Manuel adımlar:
1) qm start 101
2) Console > installer içinde SADECE ${TRUENAS_OS_DISK}GB OS diskini seç
3) Kurulum bitince:
   qm stop 101
...
6) TrueNAS Shell API key:
   midclt call api_key.create '{"name":"homelabv239","username":"truenas_admin"}'
7) Sonra Proxmox shell:
