...
echo
echo "🎛️ Homelab v2.3.13 - Bazarr / Media Management Configurator"
echo
...
DB="/opt/homelab/arr/config/bazarr/db/bazarr.db"
if [[ ! -f "$DB" ]]; then
  echo "⚠️ Bazarr DB bulunamadı, announcement cleanup atlandı: $DB"
  exit 0
fi
...
for t in tables:
    ...
    elif 'system' in tl or 'event' in tl:
        try:
            cur.execute(f'PRAGMA table_info("{t}")')
            cols = [r[1] for r in cur.fetchall()]
            for col in cols:
                if col.lower() in ('read', 'isread', 'seen', 'dismissed'):
                    cur.execute(f'UPDATE "{t}" SET "{col}"=1')
                    print(f"✅ Read/dismissed işaretlendi: {t}.{col}")
        except Exception:
            pass
conn.commit()
conn.close()
