...
occ config:system:set trusted_proxies 0 --value="172.16.0.0/12" || true
occ config:system:set trusted_proxies 1 --value="10.0.0.0/8" || true
occ config:system:set trusted_proxies 2 --value="192.168.0.0/16" || true
...
echo "📋 Kısa doğrulama:"
for key in mail_smtpmode mail_smtphost mail_smtpport mail_smtpsecure mail_smtpauth mail_smtpname mail_from_address mail_domain default_phone_region maintenance_window_start overwrite.cli.url overwriteprotocol; do
  printf '  %-26s ' "$key"
