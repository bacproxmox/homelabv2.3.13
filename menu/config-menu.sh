...
run_all_core(){
  echo
  echo "🧩 Run all core config scripts başlıyor. v2.3.13 artık tek hata yüzünden kalan scriptleri atlamaz."
  local -a failures=()
  run_step(){
    local script="$1"
...
8) Configure Bacscloud local/users + SMTP + hardening
...
MENU
  ...
  case "$choice" in
    ...
    8) run config/nextcloud/02-nextcloud-smtp-google-and-users.sh; run config/nextcloud/03-nextcloud-smtp-config.sh; run config/nextcloud/04-bacscloud-production-hardening.sh ;;
