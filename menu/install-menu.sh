...
TrueNAS API bootstrap için şu adımlar bitmiş olmalı:
  ...
  ✅ API key alınmış:
     midclt call api_key.create '{"name":"homelabv2313","username":"truenas_admin"}'
...
MENU
  ...
  case "$choice" in
    ...
    7)
      ...
      run config/nextcloud/04-bacscloud-production-hardening.sh || true
