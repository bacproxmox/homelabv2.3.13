...
TRUENAS_ENV="$SECRETS_DIR/truenas.env"
...
echo
echo "🧊 Homelab v2.3.13 - TrueNAS users + datasets + NFS + SMB"
echo

[[ -f "$USERS_ENV" ]] || {
  echo "❌ $USERS_ENV yok. Önce 01-create-users.sh çalıştır."
  exit 1
}
...
if [[ ! -f "$TRUENAS_ENV" ]]; then
  echo "TrueNAS API key oluşturmak için TrueNAS Shell:"
  echo
  echo "midclt call api_key.create '{\"name\":\"homelabv239\",\"username\":\"truenas_admin\"}'"
  echo
  echo "⚠️ API key ekranda sadece bir kez görünür."
  echo

  ask_visible TRUENAS_API_KEY "TrueNAS API key yapıştır"

  cat > "$TRUENAS_ENV" <<EOF
TRUENAS_IP="${TRUENAS_IP_DEFAULT}"
TRUENAS_API_KEY="${TRUENAS_API_KEY}"
EOF

  chmod 600 "$TRUENAS_ENV"
fi
